...
features_to_keep = ['soascaseid',
 'patid', 'sex',
 'weight',
 'height','birthday', 'sbp',
 'dbp', 'donsex','dontype','donage', 'LD_DD', 'tpxdate','DaysPostTRPLastVisit',
 'ColdIschemia', 'warmisch','A_MM', 'B_MM','C_MM','DRB1_MM','DRB345_MM','DQB1_MM','DPB1_MM', 'HLA_MM_Class I',
 'HLA_MM_Class II','HLA_MM_Total','HLA_antibodies','HLARiskGroup', 'MFI',"C/R",
                   'DSAcount',
 'DSAclass',
 'DSAcumulativeMFI', 'simult']
outcomes_to_keep = ['soascaseid',
 'patid','tpxdate', 'dgf',
 'txFailureDate',
 'txF_DaysPost_tpx',
 'deathDate',
...
def imputation(df, column_name):
    tmp = df.copy()
    if column_name in ["DSAclass", "C/R",]:
...
def create_dfs():
    ...
    for col in ["tpxdate", "birthday", "txFailureDate", "deathDate", "FirstRejectionDate"]:
        df_s[col] = pd.to_datetime(df_s[col], dayfirst = True)
    df_s_small = df_s[list(set(features_to_keep + outcomes_to_keep))]
    df_s_small["age_at_transp"] =  (df_s_small["tpxdate"]-df_s_small["birthday"]).apply(lambda x: x.days)
    ...
    for i in range(len(df_s_small.dtypes)):
        if df_s_small.dtypes[i] == "object" and (df_s_small.dtypes.index[i] not in ["soascaseid"]):
            ...
            elif name in ["Chronic active TCMR", "Chronic active ABMR", "Acute_Mixed ", "Chronic_Mixed ", "Acute/active ABMR"]:
...
def T_E_outcomes(DF0, divide=365):
    ...
    days_post = ['Rj_DaysPost_tpx', 'txF_DaysPost_tpx', 'Death_DaysPost_tpx','DaysPostTRPLastVisit']
    
    # possible outcomes
    outcomes = ['failure', 'death', 'rejection', 'ABMR', 'TCMR']
    
    var_names = {'failure': 'txF_DaysPost_tpx',
    ...
    for j,out in enumerate(outcomes):
        ...
        if out in ['failure', 'death', 'rejection']:
            ...
            E = (~DF0['Acute/active ABMR'].isnull().to_numpy()) | (~DF0['Chronic active ABMR'].isnull().to_numpy())
            ...
            E = (~DF0['Acute TCMR'].isnull().to_numpy()) | (~DF0['Chronic active TCMR'].isnull().to_numpy())
