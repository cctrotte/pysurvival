...
def display_loss_values(model, figure_size=(18, 5)):
    ...
    * figure_size: tuple of double (default= (18, 5))
    ...
    if order == 1:
        ...
        fig.suptitle(
            "Loss function $l$ and $|| gradient ||_{L_{2}}$",
            fontsize=12,
            fontweight="bold",
        )
        ...
        ax2 = ax1.twinx()
        pl2 = ax2.plot(
            range(len(grad2_values)),
            grad2_values,
            label="$|| gradient ||_{L_{2}}$",
            color="red",
        )
...
def integrated_brier_score(
    model, X, T, E, t_max=None, use_mean_point=True, figure_size=(20, 6.5)
):
...
def create_risk_groups(
    model, X, use_log=True, num_bins=50, figure_size=(20, 8), **kwargs
):
    """
    ...
    * kwargs: dict (optional)
    ...
    # Applying any color coding
        ...
        for group_name, group_def in kwargs.items():
            ...
            if (
                group_def["lower_bound"] < min_bin
                and group_def["upper_bound"] < min_bin
            ) or (
                group_def["lower_bound"] > max_bin
                and group_def["upper_bound"] > max_bin
            ):
                is_not_valid += 1
