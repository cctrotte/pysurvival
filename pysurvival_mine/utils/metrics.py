...
def concordance_index(
    model, X, T, E, include_ties=True, additional_results=False, **kwargs
):
    """
...
def other_metrics(
    ...
    for metric in metrics:
        out_survival = model_wrapper.predict_survival_mine(
            model, X_valid, T=T_train, E=E_train, X_original=X_original
        )
        indices = []
        for t in eval_times:
            min_index = [abs(a_j_1 - t) for (a_j_1, a_j) in model_wrapper.time_buckets]
...
def concordance_index_mine(
    ...
    * additional_results: bool (default=False)
    ...
    # Checking the format of the data
    if risk is None:
        risk = model_wrapper.predict_risk_mine(model, X, **kwargs)
        risk, T, E = utils.check_data(risk, T, E)
