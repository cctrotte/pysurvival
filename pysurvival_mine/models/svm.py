...
class SurvivalSVMModel(BaseModel):
    ...
    * degree: float (default=1)
    ...
    def __init__(
        self, kernel="linear", scale=1.0, offset=0.0, degree=1.0, auto_scaler=True
    ):
        ...
        if not (
            degree >= 0.0 and (isinstance(degree, float) or isinstance(degree, int))
        ):
...
class KernelSVMModel(SurvivalSVMModel):
    def __init__(
        self, kernel="gaussian", scale=1.0, offset=0.0, degree=1.0, auto_scaler=True
    ):
