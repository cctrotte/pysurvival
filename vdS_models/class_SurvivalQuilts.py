# stolen from https://github.com/vanderschaarlab/mlforhealthlabpub/tree/main/alg/survivalquilts
import warnings
...
# user defined
from vdS_models.class_UnderlyingModels import (
    CoxPH,
    CoxPHRidge,
    Weibull,
    LogNormal,
...
class SurvivalQuilts:
    ...
    def train(self, X, T, Y):
        ...
        self.all_time_horizons = [
            t
            for t in range(
                int(np.min(T[Y.iloc[:, 0] == 1])), int(np.max(T[Y.iloc[:, 0] == 1]))
            )
        ]
        ...
        metric_CINDEX, metric_BRIER = np.zeros([self.num_cv, self.M, self.K]), np.zeros(
            [self.num_cv, self.M, self.K]
        )
        self.CV_pulled_models = []

        for cv_idx in range(self.num_cv):
            print("CV.. {}/{}".format(cv_idx + 1, self.num_cv))
            pulled_models, tmp_CINDEX, tmp_BRIER = self._get_models_pulled_CV(
                X, T, Y, seed=cv_idx
            )
        ...
        X_inits[
            0, np.argmax(np.mean(metric_CINDEX, axis=0)[:, 0])
        ] = 1  # put more weights on the "best" one (at the first step)
        ...
        for k in range(self.K):
            ...
            X_inits = np.zeros([1, self.M])
            X_inits[
                0, np.argmax(np.mean(metric_CINDEX, axis=0)[:, k])
            ] = 1  # put more weights on the "best" one (at the first step)
            X_inits = self._get_normalized_X_step(X_inits)

            beta_ = np.median(
                np.mean(
                    np.mean(metric_BRIER, axis=0)[:, k : (k + self.step_ahead + 1)],
                    axis=1,
                )
            )
            ...
            for out_itr in range(self.num_outer):
                ...
                if GP_ens is not None:
                    ...
                    if (
                        max(0, tmp_c[0] - beta_) < 0.005 * beta_
                    ):  # 1% off from the median
                        print("====================================")
                ...
                lmbda_ = max(0, lmbda_ + 1.0 / rho_ * tmp_c[0])
                ...
                print(
                    "out_itr: {} | Lambda: {} | Rho: {}".format(out_itr, lmbda_, rho_)
                )

            thres_split = abs(tmp_o_prev[0] * 0.005)  # 2% improvement -> update
            if (
                -(tmp_o[0] - tmp_o_prev[0]) > thres_split
            ):  # since tmp_o is negative C-index
    ...
    def predict(self, X, eval_time_horizons=None):
        ...
        output: risk
        """
        pred_all = self._get_ensemble_prediction(
            self.underlying_models, self.quilting_patterns, X, self.all_time_horizons
        )
        ...
        if eval_time_horizons:
            ...
            for t, eval_time in enumerate(eval_time_horizons):
                pred[:, t] = pred_all[
                    :, np.where(np.asarray(self.all_time_horizons) <= eval_time)[0][-1]
                ]
        else:
        ...
        for m in range(self.M):
            ...
            if m == 0:
                ...
                for tt in range(self.K):
                    if tt == 0:
                        tmp_time_idx1 = (
                            np.asarray(all_time_horizons_) <= self.time_horizons[tt]
                        )
                        tmp_time_idx2 = (
                            np.asarray(all_time_horizons_) > self.time_horizons[tt]
                        )

                        increment = tmp_pred_[:, tmp_time_idx1] - np.matmul(
                            tmp_pred_[:, tmp_time_idx1][:, [0]],
                            np.ones([1, np.sum(tmp_time_idx1)]),
                        )

                        pred_[:, tmp_time_idx1] = (
                            pred_[:, tmp_time_idx1] + W_[tt, m] * increment
                        )
                        pred_[:, tmp_time_idx2] = pred_[:, tmp_time_idx2] + W_[
                            tt, m
                        ] * np.matmul(
                            increment[:, [-1]], np.ones([1, np.sum(tmp_time_idx2)])
                        )

                    elif tt == (self.K - 1):  # the last index
                        tmp_time_idx1 = (
                            np.asarray(all_time_horizons_) > self.time_horizons[tt - 1]
                        )
    ...
    def _get_Y_step_pulled(self, W_, X_, T_, Y_, K_step):
        ...
        for cv_idx in range(self.num_cv):
            _, X_va, T_tr, T_va, Y_tr, Y_va = train_test_split(
                X_, T_, Y_, test_size=0.20, random_state=cv_idx + self.SEED
            )

            pred = self._get_ensemble_prediction(
                self.CV_pulled_models[cv_idx], W_, X_va, self.time_horizons
            )

            new_K_step = min(K_step + 1 + self.step_ahead, self.K)
            for k in range(K_step, new_K_step):
                eval_time = self.time_horizons[k]
                tmp_C, tmp_B = calc_metrics(
                    T_tr, Y_tr, T_va, Y_va, pred[:, k], eval_time
                )

                metric_CINDEX_[cv_idx] += 1.0 / len(self.time_horizons) * tmp_C
                metric_BRIER_[cv_idx] += 1.0 / len(self.time_horizons) * tmp_B

                metric_CINDEX_[cv_idx] += 1.0 / (new_K_step - K_step) * tmp_C
                metric_BRIER_[cv_idx] += 1.0 / (new_K_step - K_step) * tmp_B

        Output_CINDEX = (
            -metric_CINDEX_.mean(),
            1.96 * np.std(metric_CINDEX_) / np.sqrt(self.num_cv),
        )
        Output_BRIER = (
            metric_BRIER_.mean(),
            1.96 * np.std(metric_BRIER_) / np.sqrt(self.num_cv),
        )
    ...
    def _get_models_pulled_CV(self, X, T, Y, seed):
        X_tr, X_va, T_tr, T_va, Y_tr, Y_va = train_test_split(
            X, T, Y, test_size=0.20, random_state=seed + self.SEED
        )

        pulled_models = self._get_trained_models(X_tr, T_tr, Y_tr)

        metric_CINDEX, metric_BRIER = np.zeros([self.M, self.K]), np.zeros(
            [self.M, self.K]
        )
        ...
        for m, model in enumerate(pulled_models):
            ...
            for t, eval_time in enumerate(self.time_horizons):
                tmp_C, tmp_B = calc_metrics(
                    T_tr, Y_tr, T_va, Y_va, pred[:, t], eval_time
                )
    ...
    def _get_AL_constraint(self, g, beta_, lmbda_, rho_):
        return np.asarray(
            lmbda_ * (g - beta_) + 0.5 / rho_ * max(0, (g - beta_)) ** 2
        ).reshape([-1, 1])
