# stolen from https://github.com/vanderschaarlab/mlforhealthlabpub/blob/main/alg/survivalquilts
...
class sksurvSurvival(object):
    ...
    def predict(self, X, time_horizons):
        if self.name in ["CoxPH", "CoxPHRidge"]:
            surv = self.model.predict_survival_function(
                X
            ...
            surv_times = self.model.unique_times_
            preds_ = np.zeros([np.shape(surv)[0], len(time_horizons)])
...
class RandomSurvForest(sksurvSurvival):
    ...
    def get_hyperparameter_space(self):
        hyp_ = [
            {
                "name": "RandomSurvForest.n_estimators",
                "type": "continuous",
                "domain": (50, 500),
                "dimensionality": 1,
            }
